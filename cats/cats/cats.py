...
def accuracy(typed, source):
    ...
    if len(typed_words)==0 or len(source_words)==0:
        ...
        for i in range(min(len(source_words),s)):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        ...
        if diff_function(typed_word,word_list[i],limit)>limit:
            num+=1
        if minn>diff_function(typed_word,word_list[i],limit):
            minn=diff_function(typed_word,word_list[i],limit)
    if num==len(word_list):
        return typed_word
    else:
        for i in range(len(word_list)):
            if diff_function(typed_word,word_list[i],limit)==minn:
                return word_list[i]
    # END PROBLEM 5
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(1,len(timestamps_per_player[i])):
            list+=[timestamps_per_player[i][j]-timestamps_per_player[i][j-1]]
        times+=[list]
    # END PROBLEM 9
